...
public class ButtonContainer extends JPanel implements ActionListener {
	Manager atariManager = new GameManager();
	JPanel buttonsPanel;
	ControllerLayout layout;
	ArrayList<JButton> buttons = new ArrayList<JButton>();
	ArrayList<Command> commands = new ArrayList<Command>();

	public ButtonContainer(JPanel buttonsPanel, GameManager gameManager, ControllerLayout layout) {
		this.atariManager = gameManager;
		this.layout = layout;
	...
	public void actionPerformed(ActionEvent arg0) {
		for (int i = 0; i < buttons.size(); i++) {
			...
			if (arg0.getSource() == buttons.get(i)) {
				((GameManager) atariManager).notifyComponents(commands.get(i));
			}
		}
